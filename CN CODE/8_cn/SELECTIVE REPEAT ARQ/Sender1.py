...
def main():
    ...
    for i in range(len(framesToSend)):
        windowToSend = framesToSend[i]
        print('\n   .....Sending the ' + str(i) + 'th window...\n')
        time.sleep(1)
        for j in range(len(windowToSend)):
            messageToSend = windowToSend[j]
            print("\nSending Frame Number " + str(j))
            ...
            while True:
                ...
                if(ack.decode('utf-8') != '1'):
                    print('ACKNOWLEDGEMENT delay...so Sending the frame '+ str(j) + ' again')
                    continue
        ...
        if(i<len(framesToSend)):
            Input = input('Press 1 to send the next window  : ')
            if(Input != '1'):
                break
